// This file is distributed under GNU GPLv3 license. For full license text, see
// <project-git-repository-root-folder>/LICENSE.md.

#include "example/gui/texteditor.h"
...
TextEditor::TextEditor(HexEditApplication* text_app, QWidget* parent)
    : QWidget(parent)
...
void TextEditor::loadFromFile(const QByteArray& file_data, const QString& file_path, const QString& encoding,
                              int initial_eol_mode)
{
    ...
    {
        qCritical().noquote().nospace() << QSL("We do not have codec for encoding '") << encoding
    ...
    QTextStream str(file_data);
    str.setCodec(codec_for_encoding);
...
void TextEditor::findAllFromSelectedText()
{
    ...
    {
        m_textApp->outputSidebar()->displayOutput(
            OutputSource::Application, tr("No other occurrences of \"%1\" found.").arg(QString(getSelText())),
...
void TextEditor::onFileExternallyChanged(const QString& file_path)
{
    ...
    if (QFile::exists(file_path))
    {
        ...
        if (m_textApp->settings()->reloadModifiedDocumentsAutomatically() ||
            MessageBox::show(
                qApp->mainFormWidget(), QMessageBox::Icon::Question, tr("File Externally Modified"),
                ...
                QMessageBox::StandardButton::Yes, &not_again,
                tr("Reload all files automatically (discard changes)")) == QMessageBox::StandardButton::Yes)
        {

            reloadFromDisk();

            qApp->showGuiMessage(tr("File '%1' was reloaded from disk.").arg(QDir::toNativeSeparators(file_path)),
                                 QMessageBox::Icon::Warning, QUrl::fromLocalFile(file_path), this);
...
void TextEditor::onModified(int type, int position, int length, int lines_added, const QByteArray& text, int line,
                            int fold_now, int fold_prev)
{
    Q_UNUSED(position)
    Q_UNUSED(length)
    Q_UNUSED(type)
...
void TextEditor::contextMenuEvent(QContextMenuEvent* event)
{
    ...
    context_menu
        .addAction(qApp->icons()->fromTheme(QSL("edit-find")), tr("&Find All"), this,
                   &TextEditor::findAllFromSelectedText)
    ...
    context_menu.addSeparator();
    context_menu
        .addAction(qApp->icons()->fromTheme(QSL("edit-select-all")), tr("&Select All"), [this]() { selectAll(); })
        ->setEnabled(length() > 0);
    context_menu.addAction(qApp->icons()->fromTheme(QSL("edit-cut")), tr("&Cut"), [this]() { cut(); })
        ->setEnabled(!selectionEmpty());
    context_menu.addAction(qApp->icons()->fromTheme(QSL("edit-copy")), tr("&Copy"), [this]() { copy(); })
        ->setEnabled(!selectionEmpty());
    context_menu.addAction(qApp->icons()->fromTheme(QSL("edit-paste")), tr("&Paste"), [this]() { paste(); })
        ->setEnabled(canPaste());
...
void TextEditor::wheelEvent(QWheelEvent* event)
{
    ...
    else
    {
        ...
        else if ((QGuiApplication::keyboardModifiers() & Qt::KeyboardModifier::ShiftModifier) ==
...
{
    return MessageBox::getSaveFileName(qApp->mainFormWidget(), tr("Save File as"),
                                       m_filePath.isEmpty() ? m_textApp->settings()->loadSaveDefaultDirectory()
                                                            : m_filePath,
                                       m_textApp->settings()->syntaxHighlighting()->fileFilters(), selected_filter);
}
...
void TextEditor::appendSessionFile(const QString& file_name, bool is_nonexistent)
{
    ...
    qApp->settings()->setValue(
        GROUP(General), General::RestoreSessionFiles,
        qApp->settings()->value(GROUP(General), SETTING(General::RestoreSessionFiles)).toStringList() << file_n);
}
...
void TextEditor::askForSaveAgreement()
{
        ...
        (modify() || (!filePath().isEmpty() && !QFile::exists(filePath()))))
    {
        ...
        m_saveAgreement =
            MessageBox::show(qApp->mainFormWidget(), QMessageBox::Icon::Question, tr("Unsaved Changes"),
                             tr("This document has unsaved changes, do you want to save them?"), QString(), filePath(),
...
void TextEditor::updateOccurrencesHighlights()
{
    ...
    if (!sel_text.isEmpty())
    {
        ...
        {
            QPair<int, int> found_range =
                findText(search_flags, sel_text.constData(), first_visible_position, end_position);
            ...
            else
            {
                ...
                first_visible_position =
                    found_range.first == found_range.second ? (found_range.second + 1) : found_range.second;
            }
        }
    }
}
...
void TextEditor::reloadLexer(const Lexer& default_lexer)
{
    ...
    indicSetFore(
        INDICATOR_URL,
        color_theme.hasComponent(SyntaxColorTheme::StyleComponents::ScintillaUrlHighlight)
            ? QCOLOR_TO_SPRT(
                  color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaUrlHighlight).m_colorForeground)
            : RGB_TO_SPRT(0, 225, 0));
    indicSetFore(
        INDICATOR_FIND,
        color_theme.hasComponent(SyntaxColorTheme::StyleComponents::ScintillaOccurrencesHighlight)
            ? QCOLOR_TO_SPRT(color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaOccurrencesHighlight)
                                 .m_colorForeground)
    ...
    setWhitespaceFore(
        true, QCOLOR_TO_SPRT(
                  color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaControlChar).m_colorForeground));
    setWhitespaceSize(3);
    ...
    color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaControlChar)
        .applyToEditor(*this, STYLE_CONTROLCHAR);

    // Set selection colors.
    setSelFore(true, QCOLOR_TO_SPRT(
                         color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaPaper).m_colorBackground));
    setSelBack(true,
               QCOLOR_TO_SPRT(color_theme.component(SyntaxColorTheme::StyleComponents::Default).m_colorForeground));
    ...
    if (color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaCurrentLine).m_colorBackground.isValid())
    {
        ...
        setCaretLineBack(QCOLOR_TO_SPRT(
            color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaCurrentLine).m_colorBackground));
    }
    else
    {
    ...
    if (m_textApp->settings()->edgeLineEnabled())
    {
        ...
        setEdgeColour(
            color_theme.hasComponent(SyntaxColorTheme::StyleComponents::ScintillaEdge)
                ? QCOLOR_TO_SPRT(
                      color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaEdge).m_colorForeground)
    ...
    if (m_textApp->settings()->codeFoldingEnabled() && m_lexer.m_code != SCLEX_NULL &&
        ...
        (propertyNames().contains("fold") || m_lexer.m_supportsOldStyleFolding))
    {
        ...
        else
        {
            setFoldMarginHiColour(
                true, QCOLOR_TO_SPRT(
                          color_theme.component(SyntaxColorTheme::StyleComponents::ScintillaMargin).m_colorBackground));
        }
...
void TextEditor::saveToFile(const QString& file_path, bool& ok, const QString& encoding)
{
    ...
    {
        MessageBox::show(qApp->mainFormWidget(), QMessageBox::Icon::Critical, tr("Cannot Save to File"),
...
TextEditor* TextEditor::fromTextFile(HexEditApplication* app, const QString& file_path,
                                     const QString& explicit_encoding, const QString& explicit_filter)
{
    try
    {
        ...
        {
            qWarningNN << QSL("File '") << file_path << QSL("' is symlink with target at '") << nfo.symLinkTarget()
                       << QSL("'.");
        ...
        FileMetadata metadata =
            FileMetadata::getInitialMetadata(file_data.first, file_target_path, explicit_encoding, explicit_filter);

        if (!metadata.m_encoding.isEmpty())
        {
            auto* new_editor = new TextEditor(app, qApp->mainFormWidget());

            new_editor->loadFromFile(file_data.first, file_target_path, metadata.m_encoding, metadata.m_lexer,
                                     metadata.m_eolMode);
    ...
    catch (const ApplicationException& ex)
    {
        QMessageBox::critical(qApp->mainFormWidget(), QObject::tr("Cannot read file"),
                              QObject::tr("File '%1' cannot be opened for reading, reason: %2.")
                                  .arg(QDir::toNativeSeparators(file_path), ex.message()));
        return nullptr;
    }
}
...
void TextEditor::reloadFromDisk()
{
    if (!filePath().isEmpty())
    {
        if (modify())
        {
            QMessageBox::StandardButton answer = MessageBox::show(
                qApp->mainFormWidget(), QMessageBox::Icon::Question, tr("Unsaved Changes"),
                tr("This document has unsaved changes, "
                   "do you want to ignore the changes and reload file?"),
                QString(), !filePath().isEmpty() ? filePath() : QString(),
        ...
        catch (const ApplicationException& ex)
        {
            QMessageBox::critical(qApp->mainFormWidget(), QObject::tr("Cannot read file"),
                                  QObject::tr("File '%1' cannot be opened for reading, reason: %2.")
                                      .arg(QDir::toNativeSeparators(filePath()), ex.message()));
        }
    }
}
