...
void QHexEdit::setHexCaps(const bool isCaps)
{
    ...
    {
        _hexCaps = isCaps;
        applyCaps(); // I wonder if this is required...
        viewport()->update();
    }
}
...
qint64 QHexEdit::indexOf(const QByteArray &ba, qint64 from)
{
    ...
    {
        qint64 curPos = pos*2;
        setCursorPosition(curPos + static_cast<qint64>(ba.length()) *2);
        resetSelection(curPos);
        setSelection(curPos + static_cast<qint64>(ba.length()) *2);
        ensureVisible();
    }
    return pos;
}
...
void QHexEdit::paintEvent(QPaintEvent *event)
{
    ...
    if (event->rect() != _cursorRect)
    {
        ...
        {
            QString address;
            for (qint64 row=0, pxPosY = _pxCharHeight; row <= (_dataShown.size()/_bytesPerLine); row++, pxPosY +=_pxCharHeight)
            {
        ...
        for (qint64 row = 0, pxPosY = pxPosStartY; row <= _rowsShown; row++, pxPosY +=_pxCharHeight)
        {
            QByteArray hex;
            int pxPosX = _pxPosHexX  - pxOfsX;
            int pxPosAsciiX2 = _pxPosAsciiX  - pxOfsX;
            qint64 bPosLine = row * _bytesPerLine;
            for (qint64 colIdx = 0; ((bPosLine + colIdx) < _dataShown.size() && (colIdx < _bytesPerLine)); colIdx++)
            {
...
void QHexEdit::applyCaps()
{
    // I really wonder if this is as efficient as I think it is. As far as I can see, we are only transforming data.
    if (hexCaps())
        _hexDataShown = _hexDataShown.toUpper();
    else
        _hexDataShown = _hexDataShown.toLower();
}
